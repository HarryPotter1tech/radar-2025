...
namespace radar.serial.crc
{
    public static class DjiCrc
    {
        ...
        {
            byte result = Crc8Init;
            foreach (var b in data)
            {
                result = Crc8Table[result ^ b];
            }
            return result;
        }

        public static bool VerifyCrc8(byte[] data)
        {
        ...
        {
            ushort result = Crc16Init;
            foreach (var b in data)
            {
                result = (ushort)((result >> 8) ^ Crc16Table[(result ^ b) & 0xff]);
            }
            return result;
        }
        ...
        public static bool VerifyCrc16(byte[] data)
        {
            ...
            ushort receivedChecksum = (ushort)((data[^1] << 8) | data[^2]);
